...
def get_fine_tuning(fine_tuning, **kwargs):
    if fine_tuning == "lora":
        target_modules=[
            ...
            "o_proj",
            "gate_proj",
        ] if "gpt2" not in kwargs["model"] else None
        return LoraConfig(
...
def prepare_taobao():
    train, test = pd.read_csv("data/taobao_train.csv"), pd.read_csv("data/taobao_test.csv")
    train = train.drop(columns=["dim_process", "seq_len"])
    test = test.drop(columns=["dim_process", "seq_len"])
    train = train.rename(
        columns={
            "seq_idx": "case_id",
            "type_event": "activity"
    ...
    sc = StandardScaler()
    columns = ["time_since_start", "time_since_last_event"]
    train.loc[:, columns] = sc.fit_transform(train[columns])
    test.loc[:, columns] = sc.transform(test[columns])
...
def main(training_config: dict):
    if training_config["log"] == "taobao":
        train, test = prepare_taobao()
        event_features = EventFeatures(
            categorical="activity",
            numerical=["time_since_start", "time_since_last_event"],
        )
        ...
        train, test = prepare_data(log.dataframe, log.unbiased_split_params) # this is my current code for the fine-tuning experiments
        event_features = EventFeatures(
...
if __name__ == "__main__":
    ...
    training_config = {
        ...
        "continuous_features": (
            NUMERICAL_FEATURES
            if (args.continuous_features is not None and "all" in args.continuous_features)
