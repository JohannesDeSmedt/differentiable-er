...
from model_help import DFGAwareTransformer, AutomatonAwareTransformer, EventTransformer
                ...
                for no_epochs in [20]:
                    for er_loss_use in [False]:
                        for d_model_p in [16 , 32, 64]:
                            for trans_model in [DFGAwareTransformer, AutomatonAwareTransformer, EventTransformer]:
                                if er_loss_use:
                                    for mix_lambda in [0.2]:
                                        model = SDFA_NAP_model(vocab_size, d_model=d_model_p, sdfa_shape=(vocab_size, vocab_size), trans_model=trans_model).to(device)
                                else:
                                    mix_lambda = 0
                                    if local:
                                        continue
                                    model = NAP_model(vocab_size, d_model=d_model_p, sdfa_shape=(vocab_size, vocab_size), trans_model=trans_model).to(device)
                                ...
                                if write:
                                    ...
                                    params={'seed': seed,  'local':local, 'quantile':quantile, 'model': model.__class__.__name__, 'lambda': mix_lambda, 
                                            'd_model': d_model_p, 'er_loss': er_loss_use, 'batch_size': bs, 'no_epochs': no_epochs, 'max_len': max_len, 'trans_model': trans_model.__name__},
